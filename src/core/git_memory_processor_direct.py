#!/usr/bin/env python3
"""
Git Memory Processor with Direct MCP Integration

This script processes Git commit data and creates individual memories for each commit
using the Graphiti MCP tool directly (bypassing the queue system), with proper GPU 
monitoring to avoid contention. Each commit becomes a discrete memory with its own 
...
def wait_for_safe_processing(check_interval: int = 5) -> bool:
    """
    Wait for Ollama to be idle (GPU monitoring only).
...
def add_memory_direct_to_graphiti(memory_name: str, episode_body: str) -> bool:
    """Add memory directly to Graphiti using the MCP tool."""
    try:
        # Use the Graphiti MCP tool directly
        from mcp_graphiti_add_memory import add_memory
        
        result = add_memory(
            name=memory_name,
            episode_body=episode_body,
            source="json",
            source_description="git commit data"
        )
...
def add_commit_to_graphiti_with_gpu_monitoring(commit_data: Dict, lmstudio_delay: int = 5) -> bool:
    """Add a single commit to Graphiti using GPU monitoring and direct MCP tool."""
    try:
        category = categorize_commit(commit_data['message'])
        ...
        # Add memory directly using MCP tool
        if add_memory_direct_to_graphiti(memory_name, episode_body):
            print(f"✅ Memory added: {memory_name}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python git_memory_processor_direct.py <repo_path> [since] [delay]")
        print("Example: python git_memory_processor_direct.py /path/to/repo '1 week ago' 5")
        sys.exit(1)
    ...
    try:
        ...
        output_file = f"git_memories_direct_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
