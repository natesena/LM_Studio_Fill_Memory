...
This is the programmatic approach to add unique memories for each commit at the precise
moment in time, as requested by the user.
"""
...
def add_commit_to_graphiti(commit_data: Dict) -> bool:
    """Add a single commit to Graphiti using the MCP tool."""
    try:
        category = categorize_commit(commit_data['message'])
        ...
        # This is where you would call the actual Graphiti MCP tool
...
def process_git_timeline_to_graphiti(repo_path: str, since: str = "1 week ago") -> List[Dict]:
    """Process Git commits and add each one to Graphiti as a separate memory."""
    print(f"Processing Git timeline from {repo_path} since {since}...")
    ...
    # Add each commit to Graphiti
    successful_memories = []
    for i, commit in enumerate(commits, 1):
        print(f"\nProcessing commit {i}/{len(commits)}: {commit['hash'][:8]}")
        
        if add_commit_to_graphiti(commit):
...
def main():
    """Main function to process Git timeline and add to Graphiti."""
    if len(sys.argv) < 2:
        print("Usage: python git_timeline_processor.py <repo_path> [since]")
        print("Example: python git_timeline_processor.py /path/to/repo '1 week ago'")
        sys.exit(1)
    ...
    try:
        successful_memories = process_git_timeline_to_graphiti(repo_path, since)
        
        # Save successful memories to file for reference
        output_file = f"git_timeline_memories_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(output_file, 'w') as f:
            json.dump(successful_memories, f, indent=2)
        
        print(f"Git timeline memory data saved to {output_file}")
        
    except Exception as e:
        print(f"Error processing Git timeline: {e}")
        sys.exit(1)
