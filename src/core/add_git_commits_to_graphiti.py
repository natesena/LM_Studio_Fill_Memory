#!/usr/bin/env python3
"""
Add Git Commits to Graphiti Knowledge Graph

This script processes Git commit data and creates individual memories for each commit
using the actual Graphiti MCP tool. Each commit becomes a discrete memory with its own
...
def add_commit_to_graphiti(commit_data: Dict) -> bool:
    ...
    try:
        ...
        # Use the actual Graphiti MCP tool
        # This would be the actual MCP tool call in a real implementation
...
def process_git_commits_to_graphiti(repo_path: str, since: str = "1 week ago") -> List[Dict]:
...
def main():
    """Main function to process Git commits and add them to Graphiti."""
    if len(sys.argv) < 2:
        print("Usage: python add_git_commits_to_graphiti.py <repo_path> [since]")
        print("Example: python add_git_commits_to_graphiti.py /path/to/repo '1 week ago'")
        sys.exit(1)
    ...
    try:
        successful_memories = process_git_commits_to_graphiti(repo_path, since)
        
        # Save successful memories to file for reference
        output_file = f"successful_git_memories_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(output_file, 'w') as f:
            json.dump(successful_memories, f, indent=2)
        
        print(f"Successful memory data saved to {output_file}")
