...
# Mock Graphiti MCP tool for now - in real implementation this would be the actual MCP tool
def add_memory_to_graphiti(name: str, episode_body: str, source: str = "git_commit", 
    ...
    """
    Add a memory to Graphiti knowledge graph.
    
    In a real implementation, this would call the actual Graphiti MCP tool.
    For now, this is a mock that prints the memory data.
    """
    memory_data = {
        "name": name,
...
def process_git_commits_to_memories(repo_path: str, since: str = "1 week ago") -> List[Dict]:
    ...
    for commit in commits:
        ...
        # Add to Graphiti (in real implementation, this would use the actual MCP tool)
        add_memory_to_graphiti(**memory_data)
    
    print(f"Created {len(memories)} memory entries.")
    return memories

def main():
    """Main function to process Git commits and create memories."""
    if len(sys.argv) < 2:
        print("Usage: python git_memory_processor.py <repo_path> [since]")
        print("Example: python git_memory_processor.py /path/to/repo '1 week ago'")
        sys.exit(1)
